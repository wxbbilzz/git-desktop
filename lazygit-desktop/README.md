...
「工作区 / 暂存区」默认用**目录树**展示，单子目录链会自动合并
（`src/components/Foo.tsx` 不会变成三层缩进）。点目录行折叠/展开，
右上角按钮可切回平铺列表。

树是复刻 lazygit `pkg/gui/filetree/build_tree.go` 的算法：插入 → 排序 → 压缩。

### 行级暂存

在**工作区**点选一个文件，diff 里的增删行会变成可勾选的：

- 点行本身 → 勾选/取消
- 点 hunk 头的「全选本块」→ 整块选中
- 顶部「暂存选中行」→ 只把选中的行进暂存区

底层复用 lazygit 的 `pkg/commands/patch`：
`Parse` 解析 → `Transform` 挑行生成子 patch → `ApplyPatch --cached` 写入索引。
所以「只提交一个文件里的部分改动」现在能直接做了。

### 提交图

历史列表左侧是**泳道图**（SVG 绘制，含合并曲线）：每条分支一种颜色，
合并提交会画出连到各父提交的弧线。

### 音效

所有操作都有对应的合成音效（Web Audio API 实时生成，不含任何音频文件）：
...
顶栏有喇叭按钮可随时静音（选择会被记住）。系统开了「减少动态效果」时，
动画会自动降级。

### 视觉

渐变光晕背景、面板玻璃质感、按钮渐变与悬停发光、选中行左侧发光条、
输入框聚焦外发光、对话框入场动画、空状态文字微光流动。
...
- **文件树**：目录结构展示，单链自动合并，可切回平铺
- **行级暂存**：勾选具体的行，只把选中的部分放进暂存区
- **提交图**：历史列表左侧的泳道图，含合并连线
- **音效**：所有操作都有合成音效，可一键静音
